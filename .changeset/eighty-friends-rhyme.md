...
- **What** Added `apiRoute` into `merkurConfig.widgetServer` to centralize the widget API endpoint name. Added `relativeUrlDefault` and `widgetParamsDefault` into `merkurConfig.playground`. `relativeUrlDefault` is used as a fallback playground path and shown in the dev server log, `widgetParamsDefault` is used to build the widget server URL for static playground and shown in the widget server log. Added a new `/merkur-integration.js` endpoint in `devServer` that serves `@merkur/integration` UMD build. Added a `merkur-integration.js` script tag loaded from the dev server. Refactored `head.ejs` template to use `Merkur.Integration.loadAssets()` for loading widget assets instead of manually rendering asset. In `footer.ejs`, added `onWidgetMounted` callback support after widget mount.
